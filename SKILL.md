...
2. **Interview for the essentials first** (ask in small batches, not all at once): name; primary goal + timeline; **preferred weight units (kg or lb)**; days/week and **typical session length** (and any shorter/longer days); training location + **equipment** available; experience level; **any injuries, pain, medical conditions, or recent labs/appointments** (and whether they're cleared for vigorous exercise); modalities they enjoy/dislike; nutrition style. Save the unit preference in the profile's "Preferred units" line — the coach prescribes loads in that unit, and `hevy_push.py` stores the exact kg so HEVY (set to the same unit) shows clean round numbers. Fill remaining template fields opportunistically over time.
...
3. **Check today's readiness, active injuries, AND recent pain/status notes.** Run `python3 scripts/injuries.py --status` for any tracked injury and its **daily 1-10 severity trend** — the bot asks the athlete each morning until it settles, so you have current data without waiting to be told. Program around any active injury; as its severity falls, **progressively reintroduce** load to that area; when it auto-resolves (two recent readings ≤2), return that area to normal (ease in moderate-first). Also run `python3 scripts/notes.py --recent 7` for any pain, injury, or status the athlete relayed (e.g. via Telegram: "lower back tight," "didn't sleep," "knee sore"). Treat these as hard constraints: **program around an injured/painful area** (avoid loading it, swap to pain-free patterns, reduce ROM/load), and **go lighter when run down**. If a note is flagged red (sharp/radiating pain, numbness, chest pain, dizziness), do NOT program that area — advise medical care first. **When status notes conflict over time, the most recent wins** — a "recovered / feeling better" note supersedes an earlier "sick / injured" note, so resume normal programming (easing in moderate-first, not straight to the hardest). For illness, apply the neck-check: above-the-neck (runny nose, mild throat cough, no fever) → light OK; below-the-neck or any fever → rest, and no HIIT/heavy. Also factor in sleep, soreness, stress, energy, and time available. This is the autoregulation input.
...
- **Telegram** — the athlete can just **text the bot**: workouts ("ran 5k in 28 min") get parsed into activities, and **pain/injury/status messages** ("lower back is tight," "didn't sleep," "knee sore") are routed to the **notes store** (`scripts/notes.py`, NOT the activity log) as constraints for the next plan — with red-flag detection (sharp/radiating/numb/chest pain → urges medical care). When the athlete reports an injury, `scripts/injuries.py` opens a tracked injury and the bot **checks in every morning** ("how's your back today, 1–10?") until it settles — so recovery is data-driven, not dependent on the athlete remembering to report. They reply with a number; it records the trend and **auto-resolves** (returning that area to normal) once two recent readings are ≤2. `scripts/telegram_ingest.py` (polled every 15 min) handles both, de-dupes activities, and replies to confirm. It also answers **queries**: "show me today's/tomorrow's workout", "send me the push workout", "show me the week ahead", "show my progress", "show me my history / last 2 weeks" → it sends the rendered HTML, the progress report, or the unified all-source history (strength + runs + HIIT) instead of logging. Queries are detected by request verbs (show/send/get/pull up) and take precedence over logging. It also handles **plan requests** ("plan me a leg day", "generate tomorrow's push workout"): `scripts/plan_on_demand.sh` runs headless Claude in the background to generate a fresh session, push it to HEVY, and Telegram it (the bot replies "on it" immediately, since generation takes a couple minutes).
